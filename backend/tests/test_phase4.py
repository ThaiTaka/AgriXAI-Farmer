...
Three demo households (nguyenvancuong PUC-001-HB, nguyenvananh PUC-002-HB,
...
def test_mu_01_plan_written_offline_is_visible_after_sync(cuong, admin):
    ...
    push(cuong, created("plans", [plan_row(record_id, name="Kế hoạch offline sáng nay", updated_at=ms("2026-09-14", 6))]))

    mine = client.get("/plans", headers=cuong).json()
    assert any(p["id"] == record_id for p in mine)
    seen = client.get("/plans", headers=admin, params={"owner_id": me(cuong)["id"]}).json()
    assert any(p["id"] == record_id for p in seen)


def test_mu_02_same_account_two_devices_conflict_is_reported_and_resolvable(cuong):
    ...
    record_id = str(uuid.uuid4())
    push(cuong, created("plans", [plan_row(record_id, name="Bản gốc", updated_at=ms("2026-09-13"))]))

    # Device 2: edits at 10:00, syncs.
    push(cuong, {"plans": {"created": [], "updated": [plan_row(record_id, name="Sửa ở máy 2", updated_at=ms("2026-09-14", 10), created_at=ms("2026-09-13"))], "deleted": []}})

    # Device 1: edited at 09:00 (older), syncs later.
    res = push(cuong, {"plans": {"created": [], "updated": [plan_row(record_id, name="Sửa ở máy 1", updated_at=ms("2026-09-14", 9), created_at=ms("2026-09-13"))], "deleted": []}})
    ...
    current = next(p for p in client.get("/plans", headers=cuong).json() if p["id"] == record_id)
    ...
    res = push(cuong, {"plans": {"created": [], "updated": [plan_row(record_id, name="Sửa ở máy 1", updated_at=newer, created_at=ms("2026-09-13"))], "deleted": []}})
    assert res["applied"]["conflicts"] == 0 and res["conflicts"] == []
    current = next(p for p in client.get("/plans", headers=cuong).json() if p["id"] == record_id)
    assert current["scenario_name"] == "Sửa ở máy 1"


def test_mu_03_two_hours_offline_then_sync(cuong):
    ...
    push(cuong, created("plans", [plan_row(record_id, name="Ghi lúc mất mạng", updated_at=two_hours_ago)]))
    row = next(p for p in client.get("/plans", headers=cuong).json() if p["id"] == record_id)
    assert row["created_at"] == two_hours_ago
    assert row["updated_at"] == two_hours_ago


def test_mu_04_farmers_never_see_each_others_farms(cuong, anh, hai):
    ...
    cuong_plots = {p["code"] for p in client.get("/plots", headers=cuong).json()}
    anh_plots = {p["code"] for p in client.get("/plots", headers=anh).json()}
    hai_plots = {p["code"] for p in client.get("/plots", headers=hai).json()}
    assert "PUC-001-HB" in cuong_plots and "PUC-001-HB" not in anh_plots and "PUC-001-HB" not in hai_plots
    assert "PUC-002-HB" in anh_plots and "PUC-002-HB" not in cuong_plots
    ...
    for path in ("/plans", "/warehouse/summary", "/income", "/expense", "/dashboard/summary"):
        res = client.get(path, headers=cuong, params={"owner_id": anh_id})
...
def test_mu_08_delete_offline_syncs_as_soft_delete(cuong):
    record_id = str(uuid.uuid4())
    push(cuong, created("plans", [plan_row(record_id, name="Sẽ xoá", updated_at=ms("2026-09-13"))]))
    assert any(p["id"] == record_id for p in client.get("/plans", headers=cuong).json())

    res = push(cuong, {"plans": {"created": [], "updated": [], "deleted": [record_id]}})
    assert res["applied"]["deleted"] == 1
    assert not any(p["id"] == record_id for p in client.get("/plans", headers=cuong).json())
    # Another device pulling later learns about the deletion.
    pulled = client.get("/sync", headers=cuong, params={"last_pulled_at": ms("2026-09-13")}).json()
...
def test_logs_accept_a_batch_and_are_idempotent(cuong, admin):
    ...
    first = client.post("/logs", headers=cuong, json=body)
    second = client.post("/logs", headers=cuong, json=body)
    assert first.status_code == 201 and second.status_code == 201
    assert first.json()["accepted"] == [log_id] == second.json()["accepted"]

    listed = client.get("/logs", headers=admin, params={"owner_id": me(cuong)["id"]}).json()
    mine = [l for l in listed if l["id"] == log_id]
    assert len(mine) == 1
    assert mine[0]["user_id"] == me(cuong)["id"]
    assert mine[0]["reported"] is True
    assert mine[0]["action"].startswith("FertilizerCalculator")


def test_logs_are_admin_only_to_read(cuong):
    assert client.get("/logs", headers=cuong).status_code == 403


# -------------------------------- dashboard ------------------------------


def test_dashboard_summary_for_the_demo_farm(cuong):
    res = client.get("/dashboard/summary", headers=cuong, params={"year": 2026, "month": 9})
    assert res.status_code == 200
    body = res.json()
    assert body["full_name"] == "Nguyễn Văn Cường"
    ...
    # Planted 20/08 -> "Ra hoa đợt đầu" stage of the tomato protocol: 4 tasks, none ticked.
    assert body["pending_tasks"] == 4
...
def test_dashboard_admin_can_view_a_farm_and_list_users(admin, anh):
    ...
    assert {u["username"] for u in users} >= {"nguyenvancuong", "nguyenvananh", "nguyenvanhai"}
...
def test_pdf_month_with_data_downloads_an_a4_document(cuong):
    res = client.get("/reports/financials.pdf", headers=cuong, params={"year": 2026, "month": 9})
    assert res.status_code == 200
...
def test_pdf_quarter_and_validation(cuong):
    ok = client.get("/reports/financials.pdf", headers=cuong, params={"year": 2026, "quarter": 3})
    assert ok.status_code == 200
    assert client.get("/reports/financials.pdf", headers=cuong, params={"year": 2026}).status_code == 422
    assert client.get("/reports/financials.pdf", headers=cuong, params={"year": 2026, "month": 13}).status_code == 422


def test_pdf_month_without_data_says_so(cuong):
    res = client.get("/reports/financials.pdf", headers=cuong, params={"year": 2025, "month": 1})
...
def test_pdf_unicode_text_survives_the_round_trip():
    ...
    assert "Nguyễn Văn Cường" in text
...
def test_e2e_p4_01_login_dashboard_pdf(cuong):
    """Login user A → dashboard shows the shed → PDF for the month downloads."""
    dash = client.get("/dashboard/summary", headers=cuong, params={"year": 2026, "month": 9}).json()
    summary = client.get("/warehouse/summary", headers=cuong).json()
    assert dash["stock_value"] == summary["total_value"]
    assert dash["stock_kg"] == summary["total_stock_kg"]
    pdf = client.get("/reports/financials.pdf", headers=cuong, params={"year": dash["year"], "month": dash["month"]})
    assert pdf.status_code == 200 and pdf.content.startswith(b"%PDF-")


def test_e2e_p4_02_offline_expense_then_second_device_conflict_resolved(cuong):
    ...
    push(cuong, created("expense", [expense("Công tưới (máy 1, offline)", ms("2026-09-13"))]))
    push(cuong, {"expense": {"created": [], "updated": [dict(expense("Công tưới — sửa ở máy 2", ms("2026-09-14", 11)), updated_by="device-2")], "deleted": []}})
    res = push(cuong, {"expense": {"created": [], "updated": [expense("Công tưới — sửa ở máy 1 lúc offline", ms("2026-09-14", 9))], "deleted": []}})
    assert res["applied"]["conflicts"] == 1
    ...
    again = push(cuong, {"expense": {"created": [], "updated": [dict(server_copy, updated_at=server_copy["updated_at"] + 1)], "deleted": []}})
    assert again["applied"]["conflicts"] == 0
    final = next(e for e in client.get("/expense", headers=cuong).json() if e["id"] == exp_id)
    assert final["description"] == "Công tưới — sửa ở máy 2"


def test_e2e_p4_03_error_in_f1_is_logged_and_reported(cuong, admin):
